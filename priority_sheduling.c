...
void read(int n) {
    for (int i = 0; i < n; i++) 
    {
        printf("Enter the ID, burst time and priority of process %d\n",i+1);
        scanf("%d%d%d", &array[i].id,  &array[i].bt,&array[i].pty);
    }
}
...
int main() {
    ...
    sort(n);
    PRIORITY(n);
    printf("ID\tBT\tPr\tCT\tTAT\tWT\n");
    for (int i = 0; i < n; i++) {
        printf("%d\t%d\t%d\t%d\t%d\t%d\n", array[i].id, array[i].bt,array[i].pty, array[i].ct, array[i].tat,
               array[i].wt);
    }
}
